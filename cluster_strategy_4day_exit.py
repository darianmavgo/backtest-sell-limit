#!/usr/bin/env python3
"""
SPXL Cluster-Based Strategy with 4-Day Mandatory Exit

This strategy uses the 4-day price pattern clusters to predict future SPXL movements
and makes trading decisions based on historical cluster performance with a mandatory
4-day exit rule to reduce holding periods.
...
- MANDATORY EXIT: All positions must be closed after 4 trading days maximum
"""
...
class ClusterStrategy4DayExit(bt.Strategy):
    """
    Cluster-Based Trading Strategy for SPXL with 4-Day Mandatory Exit
    
    Uses pre-trained K-means clusters to classify 4-day patterns
    and make trading decisions based on cluster performance.
    All positions are automatically closed after 4 trading days.
    """
    ...
    params = (
        ...
        ('max_hold_days', 4),  # Maximum holding period in trading days
    )
    ...
    def __init__(self):
        ...
        self.days_in_position = 0  # Track days held
        ...
        self.log("Cluster-Based SPXL Strategy with 4-Day Exit Initialized")
        self.log(f"Starting Portfolio: ${self.broker.getvalue():,.2f}")
        self.log(f"Maximum Hold Period: {self.p.max_hold_days} days")
    ...
    def check_exit_conditions(self, current_price):
        """Check stop loss, take profit, and mandatory 4-day exit conditions"""
        if not self.position or not self.entry_price:
            return
        
        pnl_pct = ((current_price / self.entry_price) - 1) * 100
        
        # Mandatory 4-day exit (highest priority)
        if self.days_in_position >= self.p.max_hold_days:
            self.order = self.close()
            self.log(f"MANDATORY 4-DAY EXIT: {pnl_pct:.2f}% (Day {self.days_in_position})")
        
        # Stop loss
        elif pnl_pct <= -self.p.stop_loss * 100:
            self.order = self.close()
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                if self.entry_price and hasattr(self, 'buy_order_data'):
                    ...
                    self.log(f"Trade P&L: {pnl_pct:.2f}% (${trade_profit:.2f}) | Days Held: {self.days_in_position}")
                    self.log(f"Portfolio: ${self.broker.getvalue():,.2f}")
                    
                    # Reset position tracking after sell
                    self.days_in_position = 0
    ...
    def stop(self):
        ...
        self.log("=" * 80)
        self.log("CLUSTER STRATEGY WITH 4-DAY EXIT BACKTEST RESULTS")
        self.log("=" * 80)
    ...
    def save_trade_to_db(self, sell_order, pnl_pct, trade_profit):
        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS cluster_strategy_4day_trades (
            ...
            # Insert trade record
            cursor.execute("""
                INSERT INTO cluster_strategy_4day_trades (
...
def run_backtest():
    ...
    # Add strategy
    cerebro.addstrategy(ClusterStrategy4DayExit)
    ...
    print(f"\nStarting 4-day exit strategy backtest with ${starting_cash:,}")
    print("=" * 60)

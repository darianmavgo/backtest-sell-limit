...
// GetInsuranceTickersFromTable retrieves a list of tickers from the insurance_tickers table.
func GetInsuranceTickersFromTable(db *sql.DB) ([]string, error) {
	rows, err := db.Query("SELECT DISTINCT ticker FROM insurance_tickers")
	if err != nil {
		return nil, fmt.Errorf("failed to query tickers from insurance_tickers table: %v", err)
	}
	defer rows.Close()
...
func FillHistoricalDataHandler(w http.ResponseWriter, r *http.Request) {
	...
	if symbolsParam != "" {
		...
		// For insurance_tickers table, we need to use a special query to get the ticker column
		if tableName == "insurance_tickers" {
			symbols, err = GetInsuranceTickersFromTable(dbToUse)
		} else {
		...
		go func(s string) {
			...
			if fetchErr != nil {
				results <- struct {
					symbol string
					err    error
				}{s, fmt.Errorf("failed to fetch data: %v", fetchErr)}
				return
			}
			...
			if saveErr := saveHistoricalData(dbToSave, s, historicalData); saveErr != nil {
				results <- struct {
					symbol string
					err    error
				}{s, fmt.Errorf("failed to save data: %v", saveErr)}
				return
			}
			results <- struct {
				symbol string
